...
(function () {
  ...
  if (footer) {
    footer.innerHTML = `
      ...
      <p>Created by Aaryav &middot; Wyndham, VIC &middot; 2025 &middot; Year 8 HPE Assignment</p>
  ...
  // 1. KONAMI CODE → clown rain
  const konami = ['ArrowUp','ArrowUp','ArrowDown','ArrowDown','ArrowLeft','ArrowRight','ArrowLeft','ArrowRight','b','a'];
  let ki = 0;
  document.addEventListener('keydown', e => {
    ki = (e.key === konami[ki]) ? ki + 1 : 0;
    if (ki === konami.length) { ki = 0; launchClowns(); }
  });

  // 2. Triple-click the page title (h1) → van drives across screen
  ...
  // 7. SECRET: type "rover" anywhere → van drives across
  let typed = '';
  ...
  // 8. SECRET: hold Alt + click ANY stat number → it counts to 69 then back
    ...
    el.addEventListener('click', e => {
      if (e.altKey) {
        ...
        const interval = setInterval(() => {
          if (going) { n++; el.textContent = n; if (n >= 69) going = false; }
  ...
  function toast(msg) {
    ...
    d.textContent = msg;
    d.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#ff4500;color:white;padding:12px 18px;border-radius:8px;font-weight:bold;z-index:9998;font-size:0.95rem;box-shadow:0 4px 16px rgba(0,0,0,0.4);';
